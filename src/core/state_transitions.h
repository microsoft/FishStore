...
/// Each FishStore store can perform only one action at a time (checkpoint, recovery, garbage
...
struct SystemState {
  ...
  inline SystemState GetNextState() const {
    ...
    case Action::ParserShift:
      switch(phase) {
      case Phase::REST:
        return SystemState{ Action::ParserShift, Phase::PS_PENDING, version };
      case Phase::PS_PENDING:
        return SystemState{ Action::ParserShift, Phase::REST, version };
      default:
        assert(false);
        return SystemState(UINT64_MAX);
      }
