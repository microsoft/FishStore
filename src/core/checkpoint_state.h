...
class CheckpointState {
 public:
  typedef F file_t;
  typedef std::function<void(Status)> index_persistence_callback_t;
  typedef std::function<void(Status, uint64_t, uint32_t)> hybrid_log_persistence_callback_t;
  ...
  void InitializeIndexCheckpoint(const Guid& token, uint32_t version, uint64_t table_size,
                                 ...
                                 const index_persistence_callback_t& callback) {
    failed = false;
    index_checkpoint_started = false;
  ...
  void InitializeHybridLogCheckpoint(const Guid& token, uint32_t version, bool use_snapshot_file,
                                     ...
                                     const hybrid_log_persistence_callback_t& callback) {
    failed = false;
    index_checkpoint_started = false;
    continue_tokens.clear();
    index_token = Guid{};
  ...
  void InitializeCheckpoint(const Guid& token, uint32_t version, uint64_t table_size,
                            ...
                            const index_persistence_callback_t& index_persistence_callback_,
                            const hybrid_log_persistence_callback_t& hybrid_log_persistence_callback_) {
    failed = false;
    index_checkpoint_started = false;
    continue_tokens.clear();
    index_token = token;
  ...
  std::unordered_map<Guid, std::pair<uint64_t, uint32_t>> continue_tokens;
};
