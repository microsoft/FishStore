...
class Address {
 public:
  ...
  // Utility functions for scan pages. Based on the number of bits we used in current
  // IO level of adaptive prefetching, calculate offset, page, the head of address of
  // a scan page, and the size of a scan page.
  inline uint32_t scan_offset(uint32_t scan_page_bit) const {
    return static_cast<uint32_t>(control_ & ((1LL << scan_page_bit) - 1));
  }

  inline uint64_t scan_page(uint32_t scan_page_bit) const {
    return control_ >> scan_page_bit;
  }

  inline Address scan_page_head(uint32_t scan_page_bit) const {
    return scan_page(scan_page_bit) << scan_page_bit;
  }

  inline static uint32_t scan_page_size(uint32_t scan_page_bit) {
    return 1 << scan_page_bit;
  }
...
namespace std {
template <>
inline const fishstore::core::Address& min(const fishstore::core::Address& a,
    const fishstore::core::Address& b) {
  return (b < a) ? b : a;
}
}
