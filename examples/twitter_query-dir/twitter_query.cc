...
int main(int argc, char* argv[]) {
  ...
  parser_actions.push_back({ REGISTER_GENERAL_PSF, user_id_proj });
  parser_actions.push_back({ REGISTER_GENERAL_PSF, reply_status_id_proj });
  parser_actions.push_back({ REGISTER_GENERAL_PSF, reply_user_id_proj });
  parser_actions.push_back({ REGISTER_GENERAL_PSF, lang_proj });
  ...
  JsonGeneralScanContext scan_context_push{ lang_proj, "ko" };
  begin = std::chrono::high_resolution_clock::now();
  auto status = store.Scan(scan_context_push, callback, 1);
  store.CompletePending(true);
  end = std::chrono::high_resolution_clock::now();
  printf("Scan on `lang == ko` is done in %.6f seconds...\n",
    std::chrono::duration<double>(end - begin).count());

  JsonInlineScanContext pred_scan_context{ predicate1_id, 1 };
  begin = std::chrono::high_resolution_clock::now();
  status = store.Scan(pred_scan_context, callback, 1);
  store.CompletePending(true);
  end = std::chrono::high_resolution_clock::now();
  printf("Scan over pre-registred predicate `user.lang = ja && user.follower_count > 3000` is done in %.6f secs...\n",
    std::chrono::duration<double>(end - begin).count());

  JsonInlineScanContext pred_scan_context2{ predicate2_id, 1 };
  begin = std::chrono::high_resolution_clock::now();
  status = store.Scan(pred_scan_context2, callback, 1);
  store.CompletePending(true);
  end = std::chrono::high_resolution_clock::now();
  printf("Scan over ad-hoc predicate `in_reply_to_screen_name == realDonaldTrump && possibly_sensitive` is done in %.6f seconds...\n",
    std::chrono::duration<double>(end - begin).count());


  parser_actions.clear();
  parser_actions.push_back({ DEREGISTER_GENERAL_PSF, id_proj });
  parser_actions.push_back({ DEREGISTER_GENERAL_PSF, user_id_proj });
  parser_actions.push_back({ DEREGISTER_GENERAL_PSF, reply_status_id_proj });
  parser_actions.push_back({ DEREGISTER_GENERAL_PSF, reply_user_id_proj });
  parser_actions.push_back({ DEREGISTER_GENERAL_PSF, lang_proj });
